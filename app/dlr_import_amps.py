...
def get_kafka_setup() -> tuple:
    try:
        kafka_host = os.environ['KAFKA_IP']
    except Exception:
        log.exception('KAFKA_IP is required but has not been set.')
        sys.exit(1)

    try:
        kafka_topic = os.environ['KAFKA_TOPIC']
    except Exception:
        log.exception('KAFKA_TOPIC is required but has not been set')
        sys.exit(1)
    ...
    try:
        ...
        log.exception("Connection to kafka failed. Check enviroment variable 'KAFKA_IP' and restart the container.")
        sys.exit(1)
    ...
    try:
        ...
        ksql_config = {
            "config": [
                ...
                {"TYPE": "TABLE",
                 "NAME": f"{ksql_table}",
                 "CONFIG": ("AS SELECT MRID, LATEST_BY_OFFSET(VALUE) AS VALUE_LATEST, "
                            "LATEST_BY_OFFSET(QUALITY) AS QUALITY_LATEST, LATEST_BY_OFFSET(TIME) AS TIME_LATEST "
                            f"FROM '{ksql_stream}' GROUP BY MRID EMIT CHANGES")}
            ]
        }
